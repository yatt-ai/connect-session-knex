...
function getMysqlFastQuery(tablename, sidfieldname) {
  return (
    `insert into ${
      ...
      sidfieldname
    }, expired, sess, user_uid) values (?, ?, ?, ?) on duplicate key update expired=values(expired), sess=values(sess), user_uid=values(user_uid);`
  );
}
...
function getMssqlFastQuery(tablename, sidfieldname) {
  return (
    ...
    + 'when matched then '
    + 'update set expired = S.expired, sess = S.sess, user_uid = S.user_uid '
    + 'when not matched by target then '
    + `insert (${
      sidfieldname
    }, expired, sess, user_uid) values (S.${
      sidfieldname
    }, S.expired, S.sess, S.user_uid) `
    + 'output inserted.*;'
  );
}
